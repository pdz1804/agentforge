...
## Progress
- **2026-07-06 (Phase 8, observability)** — Run persistence + token/cost. `RunRecord` (status, answer, full trace, usage, cost) + `RunStore` (`InMemoryRunStore` default, newest-first, bounded). `POST /api/runs` now streams a `run_started` event, accumulates the trace, and **persists a record on every exit path** (compile-fail, run-fail, timeout, success, and client-disconnect via `finally`, idempotent). Token usage summed from trace events; `token_cost` from a price table (unknown model → 0). New `GET /api/runs` (newest-first summaries), `GET /api/runs/{id}`, `GET /api/runs/{id}/export`. **76 tests** (68 offline + 8 docker), ruff clean. Code-review fixes: `limit<=0` guard, persist-on-disconnect, `json.dumps` events; documented multi-agent cost under-count + unauth endpoints. **Deferred:** trace-viewer UI, Postgres-backed store, trace-size limits.
- **2026-07-06 (Phase 6, multi-agent supervisor)** — Agents-as-tools: `compile_agent` recursively compiles a supervisor's `sub_agents` and exposes each as an `ask_<id>` `SubAgentTool`, so delegation reuses the ordinary tool loop (no new graph). Cycle detection (`_visiting`) + tool-name collision guard; missing sub-agent manifest → `UnknownReferenceError`. `eval_mode` propagates across the delegation boundary via a contextvar so sub-agents stay deterministic + memory-isolated during a supervisor eval run; a sub-agent that doesn't answer surfaces as a non-ok tool result. `POST /api/runs` accepts sub-agent manifests (`agents[]`). **69 tests** (61 offline + 8 docker), ruff clean; supervisor→planner+coder delegation, cycle rejection, eval propagation, and collision all proven. **Deferred:** Agent Builder UI (Next.js), nested/inlined sub-agent tracing.
- **2026-07-06 (Phase 3b, embedding search + MCP)** — `EmbeddingSearchTool` over a `VectorStore` (`InMemoryVectorStore` cosine default, tested; `openai_embed` live-ready) with `index()` + scored `run()`; registered as `embedding_search`; `POST /api/index`. MCP connector: `build_mcp_tools` (pure adapter: MCP descriptor → `BaseTool`) + `StdioMCPConnector` (lazy `mcp` SDK, live). Added `BaseTool.json_schema()` seam so MCP tools expose their raw `inputSchema` to the LLM (providers updated to use it). **64 tests** (56 offline + 8 docker), ruff clean. Code-review fixes: per-call embed client (no stale key), strict cosine (dim-mismatch fails loud), documented stateful-MCP limitation. **Deferred:** pgvector backend (shared w/ FloraLens), live MCP-server smoke, auto-binding manifest `mcp_servers` into `compile_agent`.
- **2026-07-06 (Phase 5, long-term memory)** — `MemoryProvider` wired into the runtime: **`InMemoryMemoryProvider`** (default, tested) + optional **`Mem0MemoryProvider`** (semantic; lazy, `asyncio.to_thread` so it never blocks the loop). On a run the agent retrieves relevant memories (bucketed by scope/namespace) and injects them as system context, then persists the exchange after answering; **eval-mode is memory-isolated** (no retrieve/persist). Memory inspector API: `GET/POST/DELETE /api/memory` (scope/provider validated → 400; unauth + caller-scoped → "do not expose" note pending Phase 11). **57 tests** (49 offline + 8 docker), ruff clean; cross-run recall + namespace isolation + eval isolation proven. Code-review fixes: scope-validation 500→400, mem0 event-loop blocking, bounded InMemory growth, no in-place item mutation, debug logging on memory failures. **Deferred to Phase 5b:** durable SQLite short-term thread checkpointer (multi-turn resume); mem0 live smoke (OpenAI key available).
- **2026-07-06 (Phase 4)** — Docker code sandbox. `DockerCodeExecutor` (implements `CodeExecutor`): throwaway container per run, **deny-by-default** — no host mounts, `--network none`, read-only rootfs, tmpfs `/tmp` noexec/nosuid/nodev, non-root uid + `no-new-privileges` + `cap-drop ALL`, memory/cpu/pids limits, byte-capped output, wall-clock kill via `docker rm -f`. `CodeExecutorTool` (nonzero/timeout → recoverable ToolResult), registered as `code_executor`. `POST /api/sandbox/exec` (network forced off, bounded timeout, error boundary). Chose Docker over E2B (no key; locally testable) behind the same interface. **50 tests** (43 core incl. **8 real-Docker security-matrix** rows + 7 API), ruff clean. Security matrix PASSED: network egress, host-FS write, non-allowlisted import, fork/pids, memory bomb, infinite-loop timeout, non-root, stdout cap — all contained. Code-review fixes applied (unauth egress, host-memory bound, orphan race, root user, tmpfs exec). E2B backend + docker-socket mounting for the containerized API deferred.
- **2026-07-06 (Phase 3, partial)** — Multi-provider LLM + web search + Docker. Added `OpenAIModelProvider` and gave `AnthropicModelProvider` full **tool-use** (native schema + response parsing, split into pure testable fns); provider choice stays registry-driven (`echo`/`anthropic`/`openai` via `model.provider`). `Message` extended for tool-use round-trips (assistant `tool_calls` + `role="tool"` results); runtime assigns stable tool-call ids and converts bad-arg/tool failures into recoverable tool errors (no crash). `WebSearchTool` (Tavily, injectable backend). `apps/api/Dockerfile` + compose `api` service; `.env.example` gains OPENAI/TAVILY keys. **37 tests pass** (30 core + 7 API), ruff clean — live provider/Tavily calls covered by pure-fn tests + injectable fakes (offline). **Live Docker smoke PASSED**: `docker compose up --build api` → real OpenAI (gpt-4o-mini) agent requested `web_search` → Tavily returned live results → model answered, streamed as SSE (`model`→`tool`→`answer`→`done`). Added a general `prompts/assistant.md`. Anthropic provider is code-complete + unit-tested (not live-smoked — no key supplied). Code-review fixes applied (recoverable tool errors, stable ids, `system=null` guard). **Deferred to Phase 3b:** `EmbeddingSearchTool`+pgvector, MCP connector, `HttpFetchTool`.
...
| 3 | Built-in tools + web search + MCP connector | 1,2 | Tools, MCP | ☑ (pgvector backend + live MCP smoke deferred) |
| 4 | Code sandbox + security test matrix | 1 | **Secure sandbox + safety testing** | ☑ (Docker; E2B backend deferred) |
| 5 | Memory (mem0 + checkpointer) | 1,2 | Memory | ◐ (long-term memory done; durable checkpointer→5b) |
| 6 | Multi-agent supervisor + Agent Builder UI | 2,3,5 | Orchestration, frontend | ◐ (supervisor/sub-agents done; Builder UI deferred) |
| 7 | Live 3D execution graph | 2,6 | Three.js | ☐ |
| 8 | Traces, observability, cost accounting | 2 | Observability | ◐ (persistence+cost+API done; UI + Postgres store deferred) |
